...
@section('page_title', 'Registrar Nuevo Club')
...
<div class="max-w-3xl mx-auto space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      <h2 class="text-xl font-bold text-slate-900">Alta de Institución</h2>
      <p class="text-xs text-slate-500">Carga los datos institucionales, escudo y estadio sede.</p>
    </div>
    <a
  ...
  <div class="rounded-2xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-xs">
    <form action="{{ route('admin.clubes.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
      @csrf
      ...
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div class="sm:col-span-2">
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
            Escudo Institucional (Imagen PNG / SVG / JPG)
          </label>
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1">
              ...
              <p class="mt-1 text-[11px] text-slate-400">Recomendado formato PNG transparente, máx 2MB.</p>
            </div>
          </div>
          @error('escudo')
            <p class="mt-1 text-xs text-rose-500">{{ $message }}</p>
        ...
        <div class="sm:col-span-2">
          ...
          <textarea
            name="descripcion"
            rows="3"
            placeholder="Historia, apodo o datos de interés sobre el club..."
