...
@section('page_title', 'Registrar Equipo por Categoría')
...
<div class="max-w-2xl mx-auto space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div>
      <h2 class="text-xl font-bold text-slate-900">Alta de Plantel / Equipo</h2>
      <p class="text-xs text-slate-500">Asocia un club a una categoría competitiva.</p>
    </div>
    <a
  ...
  <div class="rounded-2xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-xs">
    <form action="{{ route('admin.equipos.store') }}" method="POST" class="space-y-6">
      @csrf
      ...
      <div class="space-y-5">
        ...
        <div>
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">
            Nombre del Equipo en el Torneo <span class="text-xs font-normal text-slate-400">(Opcional)</span>
          </label>
          <input
            type="text"
            name="nombre"
            id="nombre_equipo"
            value="{{ old('nombre') }}"
            placeholder="Ej: Colón A, Colón B (Dejar vacío para usar el nombre del club)"
          ...
          <p class="mt-1 text-[11px] text-slate-400">Si el club presenta varios planteles en la misma categoría, diferéncialos aquí.</p>
        </div>
